...
/**
 * Sprint 12.1a: normalizeForHash + nearSemanticKey moved to
 * src/utils/nearSemanticKey.ts so the lessons service can import them
 * without creating a services→qc dependency. Re-exported here so
 * existing consumers (metrics.test.ts, runBaseline.ts) keep working.
 */
export { normalizeForHash, nearSemanticKey } from '../utils/nearSemanticKey.js';
