...
export async function decideStep(params: {
  request_id: string;
  /** F2f — acting principal; authorize() gate (request → topic scope). */
  actingPrincipalId?: string | null;
  step_index: number;
  actor_id: string;
  decision: string;
}): Promise<DecideResult> {
  await assertAuthorized(params.actingPrincipalId, 'write', { kind: 'request', id: params.request_id });
...
export async function getRequest(params: {
  ...
  const pool = getDbPool();
  await assertAuthorized(params.actingPrincipalId, 'read', { kind: 'request', id: params.request_id });
