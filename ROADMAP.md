...
- **Anti-hallucination experiments (Phase 17)** — citation-forced prompt templates (shipped), selective abstention (shipped, v6–v11), Chain-of-Verification synthesizer (built + measured clean → **metric-neutral**, not productionized at ~4× cost; see `docs/qc/2026-06-18-cove-edge-ab-shelve.md`)

**Status:** Sprint 16.1–16.3 shipped; Phase 17.1 shipped. CoVe (17.2) built and
measured: the first scale A/B was invalidated by an answerer chain-of-thought
leak, which drove the **LLM chat in/out standardization** (`src/services/llm/`,
reasoning-suppression + answer extraction across all 11 chat callers). The clean
re-run showed CoVe is metric-neutral on the edge set, so it is not productionized.
Remaining metric levers are retrieval-layer (DEFERRED-034 chunk granularity,
query rewrite).
