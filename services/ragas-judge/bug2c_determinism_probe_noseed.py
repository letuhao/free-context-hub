"""Bug 2c determinism probe — NO seed pinned (control).

Companion to bug2c_determinism_probe.py. Same row, same N runs, but WITHOUT
seed in model_kwargs. If this run is non-deterministic and the seed-pinned
companion is deterministic, Bug 2c's mechanism is just "LLM at temp=0 without
seed pin still has non-trivial run-to-run variance" — well-understood, not a
bug in ragas/instructor/LM Studio. The fix (provenance via judge_seed in
manifest, shipped earlier) is the right fix.

Run:
  docker cp services/ragas-judge/bug2c_determinism_probe_noseed.py free-context-hub-ragas-judge-1:/app/
  docker exec -e OPENAI_BASE_URL=http://host.docker.internal:1234/v1 \
              -e OPENAI_API_KEY=lm-studio \
              -e JUDGE_AGENT_MODEL=mistralai/mistral-nemo-instruct-2407 \
    free-context-hub-ragas-judge-1 python //app/bug2c_determinism_probe_noseed.py
"""
...
OUTPUT = Path("/app/_bug2c_determinism_noseed_output.json")
N_RUNS = 3
...
async def run() -> None:
    ...
    print(f"[probe-noseed] model={model} @ {base}  (NO SEED PINNED — control)")
    print(f"[probe-noseed] rows={len(rows_to_run)}  N_per_row={N_RUNS}")
    print()

    # KEY DIFFERENCE: no `seed` in model_kwargs.
    chat = ChatOpenAI(
        model=model,
        base_url=base,
    ...
    # Aggregate determinism summary across all probed rows.
    nondeterministic_rows = [
        r for r in all_per_row
        if (r["distinct_stmt_hashes"] > 1 or r["distinct_decision_hashes"] > 1)
    ]
    print("=" * 60)
    print(f"DETERMINISM SUMMARY (NO SEED) — {len(rows_to_run)} rows × {N_RUNS} runs")
    print("=" * 60)
